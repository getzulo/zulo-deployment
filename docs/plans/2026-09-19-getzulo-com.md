...
| Коммерческая модель | **SaaS-подписка на хостинг** | Платформа под GPLv3 — SaaS не является распространением, копилефт не мешает |
...
Честный пункт, который надо написать прямо: платформа под **GPL v3**, клиент вправе
развернуть её сам. Это продаётся как отсутствие замка́ («ваши данные и ваша модель
переносимы»), а подписка — за хостинг, обновления, бэкапы, HA и поддержку.
...
**R5 — GPL v3 без коммерческой истории.** `zulo.one/LICENSE` — GPLv3, у воркспейса
лицензии нет вообще. Продажа хостинга этому не противоречит (SaaS ≠ распространение), но
**лицензию на бизнес-модели воркспейса надо назвать явно** до публикации — сейчас она не
определена ни в одном файле.
